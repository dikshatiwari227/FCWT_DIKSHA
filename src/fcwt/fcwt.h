...
*/
Here's the completed `fcwt.h` header file with the `calculate_linscale_array` function and the rest of the class declarations:
...
public:
    ...
    int getSupport(float scale) { return static_cast<int>(fb * scale * 3.0f); };
...
public:
    FCWT_LIBRARY_API MexicanHat(float bandwidth); // Frequency domain
    ~MexicanHat() { free(mother); };
